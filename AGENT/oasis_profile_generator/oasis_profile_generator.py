...
logger = get_logger('deepak.oasis_profile')
...
class OasisProfileGenerator:
    ...
    1. Calls Zep graph retrieval functions to get richer context
    ...
    # Group/institution entity types (need representative group persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep doesn't have a built-in hybrid search API, so we search edges and nodes separately then merge results.
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, maximally restoring existing real-world situations. You must return valid JSON format, all string values must not contain unescaped newline characters. Use English."
        return base_prompt
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Personal memory (important part of the persona, introduce this individual's connection to events, and their existing actions and reactions in events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account setup for an institution/group entity, maximally restoring existing real-world situations.
...
2. persona: Detailed account setup description (2000 characters of plain text), must include:
   ...
   - Institutional memory (important part of the institutional persona, introduce this organization's connection to events, and its existing actions and reactions in events)
...
- Institutional account communication must match its identity positioning"""
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Gender mapping (includes Chinese input mappings for backward compatibility)
        gender_map = {
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses format consistent with to_reddit_format(), ensuring OASIS can read it correctly.
