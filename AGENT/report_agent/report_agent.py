...
logger = get_logger('deepak.report_agent')
...
- It is forbidden to include both tool calls and Final Answer in a single reply
- It is forbidden to fabricate tool return results (Observation), all tool results are injected by the system
...
3. Use Markdown format (but headings are forbidden):
   ...
   - ❌ It is forbidden to use any heading syntax like #, ##, ###, ####
4. [Quote Format Rules - Must Be Standalone Paragraphs]
   Quotes must be standalone paragraphs with a blank line before and after, cannot be mixed into paragraphs:
...
7. [Emphasis Again] Do not add any headings! Use **bold** instead of subsection titles"""
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You have only called{tool_calls_count} tools, at least{min_tool_calls} are needed."
    "Please call more tools to get more simulation data before outputting Final Answer.{unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only called {tool_calls_count} tools, at least {min_tool_calls} are needed."
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately based on obtained information, output section content starting with "Final Answer:" .'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You have not yet used: {unused_list}, it is recommended to try different tools for multi-perspective information"
...
class ReportAgent:
    ...
    1. Planning phase: analyzeSimulation requirement，, plan report outline structure
    2. Generation phase: generate content section by section, each section can callToolto obtain information
    3. Reflection phase: check content completeness and accuracy
    """
    
    # Maximum Toolcall count (per section)
    MAX_TOOL_CALLS_PER_SECTION = 5
    
    # Maximum reflection rounds
    MAX_REFLECTION_ROUNDS = 3
    
    # Maximum tool call count in chat
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            ...
            # ========== Backward compatible old Tool(internally redirected to new Tool） ==========
                ...
                return f"Unknown Tool: {tool_name}。. Please use one of: Tool: insight_forge, panorama_search, quick_search"
                
        except Exception as e:
            logger.error(f"ToolExecute  failed: {tool_name}, , error: {str(e)}")
            return f"ToolExecute  failed: {str(e)}"
    
    # Valid Tool nameset, used for validation in bare JSON fallback parsing
    ...
    def _get_tools_description(self) -> str:
        """Generate Tooldescription text"""
        desc_parts = ["Available Tool："]
    ...
    def _generate_section_react(
        ...
        """
        Using ReACT pattern generate single section content
        
        ReACT loop:
        1. Thought（Thought）-  - analyze what information is needed
        2. Action（Action）-  callToolto obtain information
        3. Observation（Observation）-  - analyze ToolreturnedResult
        ...
        # Build user prompt - each completed section passed in up to 4000 chars
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                logger.warning(f"Section {section.title} iteration {iteration + 1} : LLM: LLM returned None")
                # If iterations remaining, add message and retry
            ...
            # ── Conflict handling: LLM output both Tool calland Final Answer ──
            if has_tool_calls and has_final_answer:
                conflict_retries += 1
                logger.warning(
                    f"Section {section.title} round {iteration+1}: "
                    f"LLM output both Tool calland Final Answer（iteration {conflict_retries} )"
                )
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "【Format Error] You included both Tool calland Final Answer in a single reply, which is not allowed。\n"
                            "Each reply can only do one of the following two things:\n"
                            "-  calla Tool(output a <tool_call> block, do not write Final Answer)\n"
            ...
            if has_final_answer:
                # Tool callcount insufficient, reject and require more Tool
                ...
                logger.info(f"Section {section.title}  generation complete (Tool call: {tool_calls_count} times)")
            ...
            if has_tool_calls:
                ...
                # only executing firstTool call
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM attempted to call {len(tool_calls)}  Tool，only executing first: {call['name']}")
            ...
            if tool_calls_count < min_tool_calls:
                # Tool callcount insufficient, recommend unused Tool
                unused_tools = all_tools - used_tools
                unused_hint = f"(These tools have not been used yet, recommend trying them: {', '.join(unused_tools)})" if unused_tools else ""
            ...
            # Tool callsufficient, LLM output content without "Final Answer:"  prefix
            # Directly use this content as final answer, no more idle loops
            logger.info(f"Section {section.title} no 'Final Answer:' prefix detected, directly adopting LLM output as final content (tool call: {tool_calls_count} times)")
    ...
    def generate_report(
        ...
        # Completed Section titlelist (for progress tracking)
    ...
    def chat(
        ...
        During conversation, Agent can autonomously call retrieval Toolto answer questions
        ...
        Returns:
            {
                "response": "Agentreply",
                "tool_calls": [list of called Toollist],
        ...
        for iteration in range(max_iterations):
            ...
            tool_results = []
            for call in tool_calls[:1]:  # Max 1 Tool call
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Start reading from which line (for incremental fetching, 0 means from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. 1. Remove Markdown heading lines at beginning that duplicate Section title
        2. 2. Convert all ### and lower level headings to bold text
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Because Section titleare added by the system, content should not have any headings
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        1. 1. Remove duplicate headings
        2. 2. Keep report main title (#) and Section title(##), remove other level headings (###, ####, etc.)
        3. 3. Clean excess blank lines and separators
            ...
            if heading_match:
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Section titleincorrectly used #, correct to ##
