            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but entire section may not yet be complete) -->
  ...
  try {
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const summaryMatch = block.match(/摘要:\s*"?(.+?)"?(?:\n|$)/)  // matches Chinese "summary:" from backend
        const relatedMatch = block.match(/相关事实:\s*(\d+)/)  // matches Chinese "related facts:" from backend
        return {
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - select parent_601(index 0): As a representative of parents...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*选择([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)  // matches Chinese "select" prefix from backend
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible with old format: only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer available; copy as default display only when not placeholder text
      ...
      if (quotesMatch) {
        ...
        // First try to match > "text" format
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // First try "问题X:" format (Chinese "Question X:")
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch buttons (shown only when real dual-platform answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 heading (## xxx) since section title is displayed in the outer layer
  ...
  // Clean <br> before paragraph start tags that immediately follow block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> tags are separated by paragraph content, keep numbers incrementing
