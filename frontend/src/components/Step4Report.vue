  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > curly quotation marks
        if (!quoteMatches) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after alternate "Final Answer" format with colon variants
  const chineseFinalMatch = response.match(/Final Answer[:\s]\s*\n*([\s\S]*)$/i)
